...
{
    public void Guardar(Cell[] cell, string nom_arch=null, string carpeta = "Mapas_bin") {//cambio a obtner la lista de cell desde la llmada al metodo

        //funcion para guardar los datos del scriptable object en modo binario
        BinaryFormatter Bf = new BinaryFormatter();
        //crear archivo
        FileStream file = File.Create(Obt_Dir( nom_arch, carpeta));
        //convertir a json
    ...
    //-----------------------------
    //----
    public void Cargar(string nom_arch = null, string carpeta = "Mapas_bin") {
        //carga los datos el scriptableObject
        if (File.Exists(Obt_Dir(nom_arch,carpeta)))
        {
            //variable para descifrar modo binario
            BinaryFormatter Bf = new BinaryFormatter();
            //abrir archivo
            FileStream file = File.Open(Obt_Dir(nom_arch,carpeta), FileMode.Open);
    ...
    //-----------------------------
    //----
    string Obt_Dir(string nom_arch = null,string carpeta ="Mapas_bin") {
        ...
        Debug.Log("carpeta de destino: " + folderPath);
        //devuelve nombre completo mas extension
        return string.Format("{0}/{1}.bin", folderPath, nom_compl);
    }
